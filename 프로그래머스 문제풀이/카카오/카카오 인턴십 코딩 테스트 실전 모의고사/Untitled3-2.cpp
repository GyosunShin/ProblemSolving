...
void solve(vector<string>& user, string tem, int cnt){
...
void ddfs(int start, int end){
	...
	if(start >= N && end == ffinal.size()){
		++answer;
		return;
	}
	
	
	for(int i = start ; i < ffinal.size() ; ++i){
		for(int j = 0 ; j < ffinal[i].size() ; ++j){
			bool flag = true;
			for(int k = 0 ; k < ffinal[i][j].size() ; ++k){
				if(cache[ffinal[i][j][k]]){
					flag = false;
					break;
				}
			}
			if(flag){
				for(int k = 0 ; k < ffinal[i][j].size() ; ++k){
					cache[ffinal[i][j][k]] = true;
					picked.push_back(ffinal[i][j][k]);
				}
				
				ddfs(i+1);
				
				for(int k = 0 ; k < ffinal[i][j].size() ; ++k){
					cache[ffinal[i][j][k]] = false;
					picked.pop_back();
				}
			}
...
int solution(vector<string> user_id, vector<string> banned_id) {
	...
	for(it = m.begin() ; it != m.end() ; ++it){
		solve(user_id, it->first, it->second);
	}	
